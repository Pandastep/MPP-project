...
def mask_node_features(batch, mask_ratio=0.3):
...
def feature_mask_pretrain(model, loader, device, emb_dim, num_epochs=20, mask_ratio=0.3):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(loader, desc=f"[Pretrain Epoch {epoch+1}]"):
            ...
            masked_x, original_x = mask_node_features(batch, mask_ratio)
            batch.x = masked_x
